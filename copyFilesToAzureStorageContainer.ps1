...
if ($files -ne $null -and $files.Count -gt 0)
{
    ...
    if ($CreateStorageContainer.IsPresent)
    {
        ...
        else
        {
            ...
            {
                $newContainer = New-AzureStorageContainer -Name $StorageContainer -Context $StorageContext
    ...
    if ($PSCmdlet.ShouldProcess($StorageContainer, "Copy $fileCount files"))
    {
        ...
        {
            Write-Output "Replacing within $($file.FullName) the localpath of $LocalPath" 
            $escapedLocalPath = [Regex]::Escape($LocalPath)
            $blobFileName = "$($file.FullName)" -replace $escapedLocalPath, ""

            Write-Output "Copying file $($file.FullName) to new blob with name: $blobFileName"
            try
            {
                Set-AzureStorageBlobContent -Container $StorageContainer `
                    -File "$($file.FullName)" -Blob "$blobFileName" -Context $StorageContext -Force
            }
            catch
            {
